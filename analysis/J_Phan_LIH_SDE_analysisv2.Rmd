...
```{r profileA}
ma = assay(rlog(ddsTCA))[row.names(resTCA)[1:1000],]
design <- as.data.frame(colData(ddsTCA))
resPatternA <- degPatterns(ma, design, time = "time")
...
```{r profileB}
ma = assay(rlog(ddsTCB))[row.names(resTCB)[1:1000],]
design <- as.data.frame(colData(ddsTCB))
resPatternB <- degPatterns(ma, design, time = "time")
```
# Session info
